...
var dataTree =
{
    attributes :
    {
        ...
        travelCost : ['cheap', 'cheap', 'cheap', 'cheap', 'cheap', 'standard', 'standard', 'expensive', 'expensive', 'expensive'],
...
buildTree( null, null, 0, dataTree );
...
var testTree = treeID3.root;
var flag = false;
while( testTree.children.length > 0 )
{
    for( var attribute in test )
        if( testTree.data === attribute )
        {
            for (var i = 0; i < testTree.children.length; ++i)
            {
                if( test[attribute] === testTree.children[i].data )
                {
                    //console.log( testTree.children[i] );
                    if( testTree.children[i].children[0].data === 'bus' || testTree.children[i].children[0].data === 'train' || testTree.children[i].children[0].data === 'car' ) {
                        flag = true;
                    }
                    testTree = testTree.children[i].children[0];
                    break;
                }
            }
            break;
